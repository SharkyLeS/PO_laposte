...
begin_dates <- rep(first_date,nb_mois_donnees*30*24/interval-216)
...
write.csv(predictions, "C:/Travail A3/Projet d'option/Predictions_last_version/prediction_3.csv", row.names = TRUE, col.names = TRUE)

#predictions <- read.csv("C:/Travail A3/Projet d'option/Predictions_last_version/prediction_2.csv",sep=",", header=TRUE,dec=".")

predicted_data <- predictions
predicted_data <- predicted_data[481:1056,]
...
newData <- newData[1717135:1829174,]
head(newData)
tail(newData)
...
#real_data <- real_data[1:135100,]

## Reformating real data
rdata <- data.frame(matrix(ncol = length(cbns) , nrow = length(pred_row)))

#first_preddate <- as.POSIXct((real_data$date[1]), format = "%Y-%m-%d %H:%M:%S")
#begin_preddates <- rep(first_preddate,nb_intervals+1)

#for (i in 2:length(begin_preddates)){
#  begin_preddates[i] <- begin_preddates[i-1]+interval*60*60
#}
...
write.csv(rdata, "C:/Travail A3/Projet d'option/Predictions_last_version/real_3.csv", row.names = TRUE, col.names = TRUE)
